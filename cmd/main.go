...
func main() {
	...
	if err != nil {
		logger.Fatalf("创建MQTT客户端失败: %v", err)
	}

	// 连接MQTT
	if err := mqttClient.Connect(ctx); err != nil {
		logger.Fatalf("MQTT连接失败: %v", err)
	}
	defer mqttClient.Disconnect()

	logger.Infof("MQTT连接成功")
